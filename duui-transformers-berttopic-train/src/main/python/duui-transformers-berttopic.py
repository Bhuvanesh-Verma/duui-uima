...
config = Config(annotator_name=config['annotation']['name'],annotator_version=str(config['annotation']['version']),
                ...
                save_location=config['model']['save_location'])
...
def post_process(request: DUUIRequest):
    for selection in request.selections:
        for s in selection.sentences:
            accumulated_texts.append(fix_unicode_problems(s.text))
    return DUUIResponse(begin=[], end=[], results=[], factors=[],
                        len_results=[], words=[], words_prob=[],
...
def train_fit():
    if not accumulated_texts:
        return JSONResponse(status_code=400,
                            content={"status": "error", "message": "No texts accumulated"})
    topic_model = BERTopic(embedding_model="BAAI/bge-small-en")
    topic_model.fit(accumulated_texts)
